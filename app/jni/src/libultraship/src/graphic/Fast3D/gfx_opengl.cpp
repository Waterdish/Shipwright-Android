...
void logOpenGLES2Errors(const char* operation) {
    GLenum error;
    while ((error = glGetError()) != GL_NO_ERROR) {
        ...
        SDL_Log("OpenGL ES Error in %s: %s", operation, errorString);
    }
}
...
static void gfx_opengl_set_uniforms(struct ShaderProgram* prg) {
    ...
    //logopenGLES2Errors("frame_count");
    glUniform1f(prg->noise_scale_location, current_noise_scale);
    //logopenGLES2Errors("noise scale");
}
...
static void gfx_opengl_load_shader(struct ShaderProgram* new_prg) {
    ...
    //logopenGLES2Errors("use program");
    gfx_opengl_vertex_array_set_attribs(new_prg);
    //logopenGLES2Errors("set attribs");
    gfx_opengl_set_uniforms(new_prg);
    //logopenGLES2Errors("set uniforms");
}
...
static struct ShaderProgram* gfx_opengl_create_and_load_new_shader(uint64_t shader_id0, uint32_t shader_id1) {
    //logopenGLES2Errors("beginning of create shader");
    ...
    //Alright wtf bro. This needs to go before we load the shader. Lol dumb ass code.
    prg->frame_count_location = glGetUniformLocation(shader_program, "frame_count");
    prg->noise_scale_location = glGetUniformLocation(shader_program, "noise_scale");

    //logopenGLES2Errors("just before load shader");

    gfx_opengl_load_shader(prg);
    //logopenGLES2Errors("L");
...
static void gfx_opengl_draw_triangles(float buf_vbo[], size_t buf_vbo_len, size_t buf_vbo_num_tris) {
    ...
    ////SDL_Log("this is where it crashes");
...
static void gfx_opengl_init(void) {
    ...
    //SDL_Log("buffer gened");
    glBindBuffer(GL_ARRAY_BUFFER, opengl_vbo);
    //SDL_Log("boudn buffer");
    ...
    //SDL_Log("this here");
    //glEnable(GL_DEPTH_CLAMP); //This doesn't work in OpenGL ES 3
    glEnable(GL_BLEND);
    //glEnable(0x864); //GL_DEPTH_CLAMP int
...
static void gfx_opengl_update_framebuffer_parameters(int fb_id, uint32_t width, uint32_t height, uint32_t msaa_level,
    ...
    if (fb_id != 0) {
        if (fb.width != width || fb.height != height || fb.msaa_level != msaa_level) {
            if (msaa_level <= 1) { //wasn't working before but fixed now
...
gfx_opengl_get_pixel_depth(int fb_id, const std::set<std::pair<float, float>>& coordinates) {
    ...
    if (coordinates.size() == 1) {
        ...
        //I am just commenting this out. This function will just return whatever the uninitialized value is. Idk what this is even used for but it doesn't work on opengl es 3.
        //glReadPixels(x, fb.invert_y ? fb.height - y : y, 1, 1, GL_NV_read_depth_stencil, GL_UNSIGNED_INT_24_8,
                   //  &depth_stencil_value);
